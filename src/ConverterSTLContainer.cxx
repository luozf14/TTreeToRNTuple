...
struct ContainerField
{
    ...
    std::string ntupleName;
    std::string typeName;
    Int_t childFieldTypeSize; 
    std::unique_ptr<TClass> treeBuffer;
    std::unique_ptr<unsigned char[]> ntupleBuffer;
};
...
int main(int argc, char **argv)
{
    ...
    for (auto &c1 : containerFields)
    {
        ...
        c1.childFieldTypeSize = model->GetField(c1.ntupleName)->GetSubFields()[0]->GetValueSize();
        c1.treeBuffer = std::unique_ptr<TClass>(TClass::GetClass(c1.typeName.c_str()));
        tree->SetBranchAddress(c1.treeName.c_str(), c1.treeBuffer.get()->New());
    }
    model->Freeze();
    ...
    for (auto &c1 : containerFields)
    {
        c1.ntupleBuffer = std::make_unique<unsigned char[]>(c1.childFieldTypeSize);
        entry->CaptureValueUnsafe(c1.ntupleName, c1.ntupleBuffer.get());
    }
